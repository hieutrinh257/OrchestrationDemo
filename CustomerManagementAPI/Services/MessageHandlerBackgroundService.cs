...
namespace CustomerManagementAPI.Services
{
    ...
    public class MessageHandlerBackgroundService : Microsoft.Extensions.Hosting.BackgroundService
    {
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            channel.QueueDeclare(queue: "SAGA-GMS-CustomerManagementAPI-topic-queue",
                durable: true,
                exclusive: false,
                autoDelete: false,
                arguments: null);
            
            channel.QueueBind(exchange: "SAGA-GMS-topic-exchange", queue: "SAGA-GMS-CustomerManagementAPI-topic-queue", routingKey: TopicRouteKey.CustomerServices);
            ...
            channel.BasicConsume(queue: "SAGA-GMS-CustomerManagementAPI-topic-queue", autoAck: true, consumer: consumer);
        ...
        private Task<bool> HandleEvent(BasicDeliverEventArgs ea)
        {
            ...
            return HandleMessageAsync(correlationId, serviceResponseMessageType, body);
        }
            ...
            try
            {
                switch (messageType)
                {
                    case ServiceResponseMessageType.RegisterCustomer:
                        await HandleAsync(correlationId, JsonSerializer.Deserialize<Customer>(message));
                        break;
                    case ServiceResponseMessageType.UndoRegisterCustomer:
                        await HandleAsync(correlationId, emailAddress: message);
                        break;
                }
            }
            catch(Exception ex)
        ...
        private async Task<bool> HandleAsync(string correlationId, Customer inputCustomer)
        {
            bool result = false; 
                ...
                _messagePublisher
                    ...
                    correlationId,
                        ...
                        result
                            ...
                            : PublishExternalMessageType.RegisterCustomerFailed, 
        ...
        private async Task<bool> HandleAsync(string correlationId, string emailAddress)
        {
            bool result = false; 
